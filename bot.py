...
VIDEO_DELIVERY_MODE = (os.getenv("VIDEO_DELIVERY_MODE") or "send").strip().lower()
if VIDEO_DELIVERY_MODE not in {"send", "copy"}:
    logging.warning("Unknown VIDEO_DELIVERY_MODE=%s, fallback to send", VIDEO_DELIVERY_MODE)
...
async def deliver_video(bot: Bot, user_chat_id: int, row) -> bool:
    ...
    if VIDEO_DELIVERY_MODE == "send":
        if file_id:
            try:
                await bot.send_video(chat_id=user_chat_id, video=file_id, caption=title)
                return True
            except (TelegramBadRequest, TelegramForbiddenError) as exc:
                logging.warning("send_video failed for video_id=%s file_id=%s: %s", row["id"], file_id, exc)
                try:
                    await bot.send_document(chat_id=user_chat_id, document=file_id, caption=title)
                    return True
                except (TelegramBadRequest, TelegramForbiddenError) as doc_exc:
                    logging.warning(
                        "send_document fallback failed for video_id=%s file_id=%s: %s",
                        row["id"],
                        file_id,
                        doc_exc,
                    )
        else:
            logging.warning("send mode: missing file_id for video_id=%s", row["id"])
...
async def video_view(callback: CallbackQuery) -> None:
    ...
    if not row:
        await callback.answer("Видео не найдено", show_alert=True)
        return
    await deliver_video(callback.bot, callback.message.chat.id, row)
    await callback.answer()
